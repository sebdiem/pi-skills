...
Set `BROWSER_SESSION` to a unique value before starting Chrome. This gives each agent its own Chrome instance, port, and log directory — multiple agents can browse concurrently without interfering with each other or with the user's personal Chrome.

```bash
export BROWSER_SESSION="my-task-$(date +%s)"
```

All scripts (`start.js`, `nav.js`, `eval.js`, etc.) read this env var automatically. If unset, defaults to `"default"`.
...
Start Chrome on a random free port with remote debugging.
...
```
~/.cache/agent-web/logs/<session>/YYYY-MM-DD/<targetId>.jsonl
```
